...
class GlobalController extends GetxController {
  ...
  Future<void> initializeIMSDK() async {
    try {
      ...
      final initResult = await checkIMSDKInitialized();
      if (initResult) {
        print('✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅ IM SDK 初始化成功');
        ...
        final startResult = await checkIMSDKNetwork();
        if (startResult) {
  ...
  // 初始化是异步的。只要调用就代表成功
  Future<bool> checkIMSDKInitialized() async {
    try {
      final nativeService = IOSNativeService();
      nativeService.imInitialize();
  ...
  // 网络检查是异步的。只要调用就代表成功
  Future<bool> checkIMSDKNetwork() async {
    try {
      final nativeService = IOSNativeService();
      nativeService.imStart();
      await Future.delayed(const Duration(seconds: 1));
      return true;
    } catch (e) {
      return true;
  ...
  void handleNetworkEvent(
    ...
    print(
      '🌐 GlobalController 处理网络事件: code=$eventCode, desc=$eventDescription',
    );
  ...
  Future<void> logMyPublicInfo(String userId) async {
    ...
    final nativeService = IOSNativeService();
    try {
      final result = await nativeService.imBatchGetUserPublicInfo(
        userIds: [userId],
      );
      if (result['errorCode'] == 0) {
        final dataStr = result['data'] as String? ?? '';
      ...
      if (dataStr.isNotEmpty) {
        try {
          ...
          if (data is List && data.isNotEmpty) {
            ...
            return {'errorCode': 0, 'message': '获取成功', 'data': userInfo};
          } else if (data is Map) {
            return {'errorCode': 0, 'message': '获取成功', 'data': data};
          }
        } catch (e) {
          print('⚠️ 解析用户信息失败: $e');
        }
