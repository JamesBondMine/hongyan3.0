...
class _RegisterPwdPageState extends State<RegisterPwdPage> {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            widget.account,
            style: TextStyle(fontSize: 20, color: GbsColors.titleColor),
          ),
        ],
      ),
    );
  ...
  Future<void> resetPassword() async {
    await LoginController.to.resetPassword(
      widget.registerType == RegisterType.phoneCode ? widget.account : '',
      widget.registerType == RegisterType.emailCode ? widget.account : '',
      widget.code,
    ...
    try {
      ...
      if (errorCode == 0) {
        ...
        if (data != null && data.isNotEmpty) {
          try {
            ...
            if (token != null && userMap != null) {
              ...
              // 延迟后跳转到设置用户信息页
              Get.to(
                () => RegisterInfoPage(loginWithToken: true, data: dataMap),
              );
        ...
        EasyLoading.showError(
          '注册失败: {message} (code: {code})'.tr
              .replaceAll('{message}', message)
              .replaceAll('{code}', errorCode.toString()),
        );
      } else {
        EasyLoading.showError(
          '注册失败: {message} (code: {code})'.tr
      ...
      EasyLoading.dismiss();
      EasyLoading.showError(
        '注册失败: {error}'.tr.replaceAll('{error}', e.toString()),
      );
      print('❌ 注册异常: $e');
    }
  }
