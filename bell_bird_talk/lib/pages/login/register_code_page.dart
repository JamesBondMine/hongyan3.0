        ...
        children: [
          Padding(
            ...
            child: Text(
              ...
              style: TextStyle(fontSize: 14, color: GbsColors.textPrimary),
            ),
          ),
        ],
      ),
  ...
  void _getVerifyCode(VoidCallback onVerifyCodeSuccess) async {
    try {
      ...
      final result = await _nativeService.imGetCaptcha(
        widget.account,
        type: widget.registerType == RegisterType.phoneCode
            ? 'SMS'
            : 'EMAIL', // CaptchaType: SMS=短信
      ...
      EasyLoading.dismiss();
      EasyLoading.showError(
        '发送失败: {error}'.tr.replaceAll('{error}', e.toString()),
      );
    }
  }
